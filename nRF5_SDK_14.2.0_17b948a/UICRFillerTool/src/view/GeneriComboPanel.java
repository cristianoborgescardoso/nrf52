...
public class GeneriComboPanel extends javax.swing.JPanel implements LabelFillerConteiner
{
    ...
    private static final Map<Integer, Device> DEVICE_MAP = new HashMap<>();
...
//        setLayout(new SpringLayout());
    ...
    private void updateDevice(boolean keepOnHashMap)
    {
        if (keepOnHashMap)
        {
            tempDevice = DEVICE_MAP.get(this.title);
            if (tempDevice == null)
            {
                DEVICE_MAP.put(this.title, new Device(((DeviceXml) cbName.getSelectedItem()).getId(), this.title, cbInput.getSelectedIndex()));//cbInput: 0 = input; 1 = output
            }
            else
            {
        ...
        {
            DEVICE_MAP.remove(this.title);
        }
    }

    public static List<Device> getDeviceList()
    {
        return new ArrayList<>(DEVICE_MAP.values());
    }
